...
function startMessage() {
  return [
    ...
    "",
    "Buttons under this chat do the same: Status, Full story, Notify, Currency, Wallet, Help, Stop.",
    "",
    "You control epoch notes with the Notify button.",
    "",
    `<a href="${esc(mystakeUrl())}">Full story</a> – Stake health on the Hub. We keep this chat short.`,
    "",
    howToRead(),
    "",
    DISCLAIMER
...
function helpMessage() {
  return [
    ...
    "<b>Full story</b> – Stake health page for this wallet (Validator Transparency too, when we know the vote)",
...
function fullStoryLines({ wallet, stake, vote } = {}) {
  const lines = [`<a href="${esc(mystakeUrl(wallet, stake))}">Full story</a>`];
  if (vote) {
    lines.push(`<a href="${esc(compareUrl(vote))}">Validator Transparency</a>`);
  }
  return lines;
}
...
function fullStoryMessage({ wallet, stake, vote } = {}) {
  ...
  const parts = [
    "Stake health stays short here – Full story is the Hub page, not a long Telegram dump.",
    "",
...
function telegramNext(overall, pack) {
  ...
  if (overall.tone === "risk") {
    return "Open Full story for the picture. This is a checkup, not an instruction to unstake.";
  }
  return overall.next || "";
}
...
function digestNext(overall, pack) {
  if (overall.tone === "risk") {
    return "Open Full story if you want the validator picture. We do not move SOL.";
  }
  if (overall.tone === "watch") {
