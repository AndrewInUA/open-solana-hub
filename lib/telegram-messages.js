...
function startMessage() {
  return [
    ...
    "",
    "Buttons under this chat do the same: Status, Stake story, Your validator, Notify, Currency, Wallet, Help, Stop.",
    "",
    "You control epoch notes with the Notify button.",
    "",
    `<a href="${esc(mystakeUrl(null, null, { story: true }))}">Stake story</a> – stake money on Stake health (recent rewards in this checkup). Your validator is a separate link.`,
    "",
    howToRead(),
    "",
    DISCLAIMER
...
function helpMessage() {
  return [
    ...
    "<b>Stake story</b> – stake money picture on Stake health (recent rewards in this checkup – not all time, and not the validator page)",
...
function fullStoryMessage({ wallet, stake, vote } = {}) {
  ...
  const parts = [
    "Stake story is the stake money picture on Stake health – recent rewards in this checkup, not a lifetime total, not a Telegram dump, and not the validator page.",
    "",
...
function validatorMessage({ wallet, vote, votes } = {}) {
  ...
  const extra =
    Array.isArray(votes) && votes.length > 1
      ...
      : "No vote account on file yet. Stake story is still the stake money page.";
  return [
    extra,
    "",
...
function telegramNext(overall, pack) {
  ...
  if (overall.tone === "risk") {
    return "Open Stake story for the stake money picture. This is a checkup, not an instruction to unstake.";
  }
  return overall.next || "";
}
...
function digestNext(overall, pack) {
  if (overall.tone === "risk") {
    return "Open Stake story for the stake money picture. We do not move SOL.";
  }
  if (overall.tone === "watch") {
