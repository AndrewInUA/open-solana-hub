...
function startMessage() {
  return [
    ...
    `<a href="${esc(compareUrl())}">Your validator</a> – today’s cut and recorded moves.`,
    "",
    howToRead(),
    "",
    DISCLAIMER
...
function helpMessage() {
  return [
    ...
    "<b>Your validator</b> – today’s cut and recorded moves. Telegram pings a raise. A lower cut stays on that list – no extra ping",
    "<b>Turn notes on</b> / <b>Turn notes off</b> – tap to start or stop epoch notes. A higher validator cut gets its own note. A lower cut is on Your validator – no extra ping. Unlink starts raise-watch over; cut history stays. Notes default on after you link a wallet",
...
function unlinkedOneMessage(wallet, remaining) {
  ...
  return [
    ...
    "Raise notes for a dropped wallet start over if you link it again. Cut history stays on Your validator and Stake health."
  ].join("\n");
}
...
function commissionRaiseMessage(raises) {
  ...
  lines.push("");
  lines.push("A higher cut does not undo itself. Cut history is on Your validator and Stake health.");
  lines.push("");
  for (const r of list) {
...
function compactFeeLines(pack, { dates = false, named = false } = {}) {
  ...
  if (dates) {
    ...
    if (pack.lines.length) {
      ...
      for (const line of pack.lines) lines.push(esc(line.text));
      if (pack.truncated) lines.push("Older moves are on Validator Transparency.");
...
function validatorMessage({ wallet, vote, votes, feeHistories } = {}) {
  ...
  if (histories.length) {
    const lines = [
      "Your validator – today’s cut and recorded moves. Telegram pings a raise. A lower cut stays here – no extra ping.",
      ""
    ];
  ...
  if (vote) {
    return [
      "Your validator – today’s cut and recorded moves.",
      "",
