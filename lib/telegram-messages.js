...
function startMessage() {
  return [
    ...
    "/status – how much is staked, last-epoch payout, OK / Watch / Risk",
    ...
    "",
    `<a href="${esc(mystakeUrl(null, null, { story: true }))}">Stake story</a> – last epoch payout on Stake health.`,
...
function helpMessage() {
  return [
    ...
    "",
    "<b>Status</b> – how much is staked, last-epoch payout, OK / Watch / Risk",
    "<b>Stake story</b> – last epoch payout on Stake health",
...
function fullStoryMessage({ wallet, stake, vote } = {}) {
  ...
  const parts = [
    "Last epoch payout on Stake health.",
    "",
...
function validatorMessage({ wallet, vote, votes } = {}) {
  ...
  const extra =
    Array.isArray(votes) && votes.length > 1
      ...
      : "No vote account on file yet. Stake story still opens last epoch on Stake health.";
  return [
    extra,
    "",
...
function telegramNext(overall, pack) {
  ...
  if (overall.tone === "risk") {
    return "Open Stake story for the last-epoch payout.";
  }
  return overall.next || "";
}
