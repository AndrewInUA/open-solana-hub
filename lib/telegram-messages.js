...
function helpMessage() {
  return [
    ...
    "/unlink ADDRESS  – drop one wallet",
    "/unlink  – same as Stop, drop all",
    "",
...
function linkedMessage(wallet, fiat, notifyOn = true, wallets = null) {
  ...
  const extra =
    list.length > 1
      ...
      : "Send another public key to add a second wallet. /unlink ADDRESS drops one.";
  return [
...
function walletListMessage(wallets) {
  ...
  return [
    ...
    ...lines,
    "",
    "Send another public key to add it (up to 5). /unlink ADDRESS drops one. /stop drops all."
  ].join("\n");
}
...
function walletFullMessage(wallets) {
  return [
    ...
    "/unlink ADDRESS drops one. /stop drops all.",
    "",
    DISCLAIMER
  ].join("\n");
}
