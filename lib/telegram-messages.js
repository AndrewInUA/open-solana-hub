...
const { moneyLines, mystakeUrl, compareUrl, storyContextFromView, FIAT_CODES, HOW_TO_READ, TONE_COPY, fiatFreshnessCopy } = core;
...
function startMessage() {
  return [
    ...
    "",
    "Buttons under this chat do the same: Status, Full story, Your validator, Notify, Currency, Wallet, Help, Stop.",
    "",
    "You control epoch notes with the Notify button.",
    "",
    `<a href="${esc(mystakeUrl(null, null, { story: true }))}">Full story</a> – stake money on Stake health. Your validator is a separate link.`,
    "",
    howToRead(),
    "",
    DISCLAIMER
...
function helpMessage() {
  return [
    ...
    "<b>Full story</b> – stake money picture on Stake health (what this stake earned – not the validator page)",
    "<b>Your validator</b> – Validator Transparency compare profile, when we know the vote",
    "<b>Notify</b> – turn epoch notes on or off (button shows Notify: On or Notify: Off)",
...
function notifyToggled(on) {
  return on
    ...
    : "Epoch notes are off. You will not get epoch notes. /status still works anytime. Tap Notify: Off to turn them on again.";
}
...
function fullStoryLines({ wallet, stake, vote } = {}) {
  const lines = [
    `<a href="${esc(mystakeUrl(wallet, stake, { story: true }))}">Full story</a>`
  ];
  if (vote) {
...
function fullStoryMessage({ wallet, stake, vote } = {}) {
  ...
  const parts = [
    "Full story is the stake money picture on Stake health – what this stake earned, not a Telegram dump, and not the validator page.",
    "",
...
function validatorMessage({ wallet, vote, votes } = {}) {
  ...
  if (vote) {
    return [
      "Your validator – Validator Transparency compare profile. This is not the stake money story.",
      "",
  ...
  const extra =
    Array.isArray(votes) && votes.length > 1
      ...
      : "No vote account on file yet. Full story is still the stake money page.";
  return [
    extra,
    "",
    `<a href="${esc(mystakeUrl(wallet, null, { story: true }))}">Full story</a>`,
    "",
    DISCLAIMER
  ].join("\n");
}
...
function telegramNext(overall, pack) {
  ...
  if (overall.tone === "risk") {
    return "Open Full story for the stake money picture. This is a checkup, not an instruction to unstake.";
  }
  return overall.next || "";
}
...
function digestNext(overall, pack) {
  if (overall.tone === "risk") {
    return "Open Full story for the stake money picture. We do not move SOL.";
  }
  if (overall.tone === "watch") {
