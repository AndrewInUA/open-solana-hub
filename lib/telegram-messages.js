...
const { shortKey, moneyLine, mystakeUrl, FIAT_CODES, fmtPct } = core;
...
function howToRead() {
  return [
    "<b>OK</b> – live status, fee, and stored history look ordinary. You do not need to act.",
    "<b>Watch</b> – something is off (a higher fee, a cooldown, thinner history). Read it; it is not an alarm.",
    "<b>Risk</b> – delinquent, very high commission, or a weak record. Open the full page before you decide anything."
  ].join("\n");
}
...
function helpMessage() {
  return [
    "Send /wallet plus a Solana public key, then /status any time.",
    "",
    "/wallet &lt;address&gt;",
    "/status",
...
function formatStatus(view, { fiat, wallet, kind = "status" } = {}) {
  ...
  if (Number.isFinite(Number(overall.lastEpochSol))) {
