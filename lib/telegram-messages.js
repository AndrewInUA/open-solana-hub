...
function startMessage() {
  return [
    ...
    `<a href="${esc(compareUrl())}">Your validator</a> – the operator page: voting and stability.`,
    "",
    howToRead(),
    "",
    DISCLAIMER
...
function helpMessage() {
  return [
    ...
    "<b>Your validator</b> – the operator page: voting and stability. Cut history is Fee history on Stake health",
...
function linkedMessage(wallet, fiat, notifyOn = true, wallets = null) {
  ...
  return [
    ...
    "Telegram pings a raise from now on. Older cut moves are on Stake health.",
    "You control epoch notes with Turn notes on / off.",
    "",
    DISCLAIMER
  ].join("\n");
...
function multiWalletPagesMessage(wallets, contexts = {}, { kind } = {}) {
  const intro =
    kind === "validator"
      ? "Operator page for each wallet – voting and stability:"
...
function validatorMessage({ wallet, vote, votes } = {}) {
  ...
  if (voteList.length) {
    const lines = [
      "This operator's page: voting and stability. Cut history is on Stake health.",
      ""
    ];
    for (const item of voteList) {
      lines.push(`<a href="${esc(compareUrl(item))}">Your validator</a>`);
