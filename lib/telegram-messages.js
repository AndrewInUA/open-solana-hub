...
const { moneyLines, mystakeUrl, compareUrl, storyContextFromView, FIAT_CODES, HOW_TO_READ, TONE_COPY, toneBadge, fiatFreshnessCopy, worseTone, feeHistoriesFromRows, TELEGRAM_CTA } = core;
...
function startMessage() {
  return [
    ...
    TELEGRAM_CTA.notesHow,
    "",
    `<a href="${esc(mystakeUrl(null, null, { story: true }))}">Stake story</a> – last epoch, then last epochs' rewards on Stake health.`,
    `<a href="${esc(mystakeUrl(null, null, { fee: true }))}">Fee history</a> – today’s cut and recorded moves on Stake health.`,
    `<a href="${esc(compareUrl())}">Your validator</a> – voting and stability.`,
    "",
    howToRead(),
    "",
    DISCLAIMER
...
function helpMessage() {
  return [
    ...
    "<b>Your validator</b> – operator profile. Fee history is on Stake health. Telegram pings a raise. A lower cut stays on that list – no extra ping",
    "<b>Turn notes on</b> / <b>Turn notes off</b> – tap to start or stop epoch notes. A higher validator cut gets its own note. A lower cut is on Stake health – no extra ping. Unlink starts raise-watch over; cut history stays. Notes default on after you link a wallet",
...
function linkedMessage(wallet, fiat, notifyOn = true, wallets = null) {
  ...
  return [
    ...
    "Telegram pings a raise from now on. Older cut moves are on Your validator and Stake health.",
    "You control epoch notes with Turn notes on / off.",
    "",
    DISCLAIMER
  ].join("\n");
...
function unlinkedOneMessage(wallet, remaining) {
  if (!remaining.length) {
    return "Dropped that wallet. Raise notes start over if you link again. Cut history stays on Stake health. Send a public key to add one.";
  }
  return [
    `Dropped <code>${esc(wallet)}</code>.`,
    `Still watching ${remaining.length} wallet${remaining.length === 1 ? "" : "s"}.`,
    "Raise notes for a dropped wallet start over if you link it again. Cut history stays on Stake health."
  ].join("\n");
}

function notifyToggled(on) {
  return on
    ? "Epoch notes are on. After a new epoch, the next morning (08:00 UTC) you get the checkup, and a separate note if your validator raises its cut. A lower cut is not a ping. /status still works anytime."
    : "Epoch notes are off. You will not get epoch notes or cut-change notes. /status still works anytime. Tap Turn notes on to get them again.";
}

function unlinkedMessage() {
  return "Wallets unlinked. Raise notes start over if you link again. Cut history stays on Stake health. Send /start if you want to link again.";
}
...
function commissionRaiseMessage(raises) {
  const list = (raises || []).filter(r => r && r.vote);
  if (!list.length) return "";
  const lines = [];
  if (list.length === 1) {
    const r = list[0];
    const name = r.name || "Your validator";
    lines.push(
      `<b>${esc(name)}</b> raised the validator's cut: ${esc(cutPct(r.from))} → ${esc(cutPct(r.commission))}.`
    );
  } else {
    lines.push("<b>Validator's cut went up</b> on a wallet you watch.");
    for (const r of list) {
      const name = r.name || shortWallet(r.vote);
      lines.push(`${esc(name)}: ${esc(cutPct(r.from))} → ${esc(cutPct(r.commission))}`);
    }
  }
  lines.push("");
  lines.push("A higher cut does not undo itself. Cut history is on Stake health.");
  lines.push("");
  ...
  for (const r of list) {
    ...
    lines.push(`<a href="${esc(mystakeUrl(wallet, null, { fee: true }))}">Fee history</a>`);
  }
  if (!seenWallets.size) {
    lines.push(`<a href="${esc(mystakeUrl(null, null, { fee: true }))}">Fee history</a>`);
  }
  for (const r of list) {
    const extra = r.name ? ` – ${esc(r.name)}` : "";
    lines.push(`<a href="${esc(compareUrl(r.vote))}">Your validator</a>${extra}`);
  }
...
function fullStoryLines({ wallet, stake, vote } = {}) {
  ...
  if (wallet) {
    lines.push(`<a href="${esc(mystakeUrl(wallet, stake, { fee: true }))}">Fee history</a>`);
  }
  if (vote) {
...
function compactFeeLines(pack, { dates = false, named = false } = {}) {
  if (!pack) return [];
  const lines = [];
  const telegramFeeShow = 8;
  if (dates) {
    if (pack.name) lines.push(`<b>${esc(pack.name)}</b>`);
    if (pack.nowLine) lines.push(esc(pack.nowLine));
    if (pack.lines.length) {
      lines.push(esc(pack.headline));
      for (const line of pack.lines.slice(0, telegramFeeShow)) lines.push(esc(line.text));
      if (pack.lines.length > telegramFeeShow || pack.truncated) {
        lines.push("Full list is on Stake health.");
      }
    } else if (pack.headline) {
      lines.push(esc(pack.headline));
    }
    for (const row of pack.pattern || []) {
      lines.push(esc(`${row.label}: ${row.text}`));
    }
    if (pack.votingSummary) lines.push(esc(pack.votingSummary));
    return lines;
  }
  const who = named && pack.name ? `${pack.name}: ` : "";
  const bits = [pack.nowLine, pack.headline].filter(Boolean);
  if (!bits.length) return [];
  lines.push(esc(`${who}${bits.join(" ")}`.trim()));
  return lines;
}

function feeHistoryLink(wallet) {
  return `<a href="${esc(mystakeUrl(wallet || null, null, { fee: true }))}">Fee history</a>`;
}

function validatorMessage({ wallet, vote, votes, feeHistories } = {}) {
  if (!wallet) return noWalletMessage();
  const histories = (feeHistories || []).filter(pack => pack && (pack.vote || pack.nowLine || pack.headline));
  if (histories.length) {
    const lines = [
      "Today’s cut and recorded moves. Telegram pings a raise. A lower cut stays on Stake health – no extra ping.",
      ""
    ];
    for (const pack of histories) {
      lines.push(...compactFeeLines(pack, { dates: true }));
      lines.push("");
    }
    lines.push(feeHistoryLink(wallet));
    for (const pack of histories) {
      if (!pack.vote) continue;
      const extra = pack.name ? ` – ${esc(pack.name)}` : "";
      lines.push(`<a href="${esc(compareUrl(pack.vote))}">Your validator</a>${extra}`);
    }
    lines.push("", DISCLAIMER);
    return lines.join("\n");
  }
...
function formatStatus(view, { fiat, wallet, kind = "status" } = {}) {
  ...
  const feePacks = feeHistoriesFromRows(view.rows);
  if (feePacks.length) {
    parts.push("");
    for (const pack of feePacks.slice(0, 3)) {
      parts.push(...compactFeeLines(pack, { dates: false, named: feePacks.length > 1 }));
    }
  }
...
function formatWalletBlock(view, { fiat, wallet }) {
  ...
  const feePacks = feeHistoriesFromRows(view.rows);
  const feeLines = feePacks.flatMap(pack =>
    compactFeeLines(pack, { dates: false, named: feePacks.length > 1 })
  );
  return [
