...
describe("telegram bot URL helper", () => {
  it("builds t.me links and rejects junk", () => {
    ...
    assert.equal(telegramBotUrl("@OpenSolanaHubBot"), "https://t.me/OpenSolanaHubBot");
    assert.equal(telegramBotUrl(""), null);
    assert.equal(telegramBotUrl("ab"), null);
    assert.equal(TELEGRAM_CTA.fallback.includes("coming"), true);
  });
});
...
describe("telegram-info public endpoint", () => {
  ...
  it("falls back cleanly when username is unset", async () => {
    const { status, json } = await callInfo("");
    assert.equal(status, 200);
    assert.equal(json.url, null);
    assert.equal(json.username, null);
    assert.equal(json.fallback, TELEGRAM_CTA.fallback);
  });
});
