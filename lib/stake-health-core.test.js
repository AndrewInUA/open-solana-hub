...
describe("fee history on Stake health", () => {
  ...
  it("puts commission, delinquency, stability, and voting on Stake health", () => {
    const hist = feeHistoryFromOverlay(
      overlay({
        commission: 0,
        stability: {
    ...
    assert.equal(hist.pattern[0].label, "Commission pattern");
    assert.match(hist.pattern[0].text, /Unchanged at 0%/);
    assert.match(hist.pattern[1].text, /No delinquent days in 8,213/);
    assert.match(hist.pattern[2].text, /100\/100/);
    assert.match(hist.votingSummary, /99(?:\.0)?%/);
    assert.match(hist.votingLines[0].label, /Epoch /);
  });
...
describe("telegram copy", () => {
  ...
  it("Your validator lists recorded cut moves, newest first", () => {
    ...
    assert.match(text, /Commission pattern/);
    assert.match(text, /Delinquency in snapshots/);
  });
