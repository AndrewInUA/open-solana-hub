...
describe("stake money picture (shared with mystake.js)", () => {
  ...
  it("labels payouts we actually have, not an empty 16-epoch basket", () => {
    ...
    assert.equal(since.windowLabel, "Last epochs' rewards");
    ...
    assert.equal(windowed.windowLabel, "Last epochs' rewards");
    ...
    assert.doesNotMatch(story, /from 2 payouts we could follow/);
  ...
  it("labels overall recent payouts by epoch count, not summed stake payouts", () => {
    ...
    assert.equal(money.windowLabel, "Last epochs' rewards");
    assert.match(moneyStory(money, { perSol: { USD: 150 } }, "USD"), /Last epoch/);
    assert.doesNotMatch(moneyStory(money, { perSol: { USD: 150 } }, "USD"), /from 4 payouts/);
  });
  ...
  it("lists a consecutive run from last epoch, not a gappy basket", () => {
    ...
    assert.equal(summary.windowLabel, "Last epochs' rewards");
  ...
  it("loads extra epochs for every stake in one getInflationReward batch", async () => {
    const first = "Stake11111111111111111111111111111111AAA";
    const second = "Stake11111111111111111111111111111111BBB";
    const calls = [];
    async function rpc(_method, params) {
      const keys = params[0];
      const epoch = params[1].epoch;
      calls.push({ keys: [...keys], epoch });
      if (epoch === 841) {
        return [
          { amount: 1e9, epoch: 841, postBalance: 11e9, commission: 0 },
          { amount: 5e9, epoch: 841, postBalance: 30e9, commission: 0 }
        ];
      }
      if (epoch === 840) {
        return [
          { amount: 1e9, epoch: 840, postBalance: 12e9, commission: 0 },
          { amount: 5e9, epoch: 840, postBalance: 35e9, commission: 0 }
        ];
      }
      return [null, null];
    }
    const out = await fetchRewardHistory(
      ...
      [
        acc({
          pubkey: first,
          activationEpoch: 840,
          rewards: [{ amountSol: 1, epoch: 841 }]
        }),
        acc({
          pubkey: second,
          delegatedSol: 28,
          activationEpoch: 840,
          rewards: [{ amountSol: 5, epoch: 841 }]
        })
      ],
      842
    );
    assert.equal(calls.every(c => c.keys[0] === first && c.keys[1] === second), true);
    const rates = { perSol: { USD: 150 }, source: "CoinGecko", stale: false, at: Date.now() };
    const secondMoney = summarizeAccountRewards(out[1], 842);
    assert.equal(secondMoney.showCumulative, true);
    assert.match(moneyLines(secondMoney, rates, "USD").join("\n"), /Epoch 841/);
    assert.match(moneyLines(secondMoney, rates, "USD").join("\n"), /Epoch 840/);
  });

  it("retries a flaky getInflationReward instead of treating it as a hole", async () => {
    const calls = new Map();
    async function rpc(_method, params) {
      const epoch = params[1].epoch;
      calls.set(epoch, (calls.get(epoch) || 0) + 1);
      if (epoch === 841) {
        return [{ amount: 1e9, epoch: 841, postBalance: 11e9, commission: 0 }];
      }
      if (epoch === 840) {
        if (calls.get(epoch) < 3) throw new Error("busy");
        return [{ amount: 2e9, epoch: 840, postBalance: 13e9, commission: 0 }];
      }
      return [null];
    }
    const [out] = await fetchRewardHistory(
      rpc,
      [acc({ pubkey: "Stake11111111111111111111111111111111AAA", activationEpoch: 840, rewards: [] })],
      842
    );
    assert.equal(
      out.rewards.some(r => r.epoch === 840 && r.amountSol === 2),
      true
    );
    const summary = summarizeAccountRewards(out, 842);
    assert.deepEqual(
  ...
  it("money story and telegram lines stay on Approx. copy, with en dashes", () => {
    ...
    assert.match(story, /Last epoch/);
    assert.doesNotMatch(story, /from 2 payouts|this stake started|lifetime/i);
    ...
    assert.equal(lines[2].startsWith("Last epochs' rewards:"), true);
    assert.match(lines.join("\n"), /Epoch 841/);
    assert.match(lines.join("\n"), /Epoch 840/);
    assert.doesNotMatch(lines.join("\n"), /Recent payouts|this stake started|lifetime/i);
    assert.equal(MAX_REWARD_HISTORY_EPOCHS, 16);
    assert.equal(core.REWARD_RPC_CONCURRENCY, 3);
    assert.equal(core.REWARD_RPC_RETRIES, 3);
  });
...
describe("fee history on Stake health", () => {
  it("lists newest cut change first and treats a drop as not a raise", () => {
    const hist = feeHistoryFromOverlay(
      overlay({
        commission: 0,
        stability: { score: 90, label: "Strong", sample: 40, delinquent: 0, commissionChanges: 2 },
        feeEvents: [
          { from: 0, to: 8, captured_at: "2025-01-10T00:00:00.000Z" },
          { from: 8, to: 0, captured_at: "2025-03-02T00:00:00.000Z" }
        ]
      })
    );
    assert.match(hist.nowLine, /Today.s cut: 0%/);
    assert.match(hist.headline, /2 recorded cut changes/);
    assert.equal(hist.lines.length, 2);
    assert.equal(hist.lines[0].raised, false);
    assert.equal(hist.lines[0].tone, "ok");
    assert.match(hist.lines[0].text, /8% → 0%/);
    assert.equal(hist.lines[1].raised, true);
    assert.equal(hist.lines[1].tone, "watch");
    assert.match(hist.lines[1].text, /0% → 8%/);
    assert.match(hist.lines[0].text, / – /);
  });

  it("keeps the stored cut list on Stake health, not a short sample", () => {
    const feeEvents = [];
    for (let i = 0; i < 12; i++) {
      feeEvents.push({
        from: i,
        to: i + 1,
        captured_at: `2025-01-${String(i + 1).padStart(2, "0")}T00:00:00.000Z`
      });
    }
    const hist = feeHistoryFromOverlay(
      overlay({
        commission: 12,
        stability: { score: 80, label: "OK", sample: 40, delinquent: 0, commissionChanges: 12 },
        feeEvents
      })
    );
    assert.equal(hist.lines.length, 12);
    assert.equal(hist.truncated, false);
  });

  it("says unchanged when stored snapshots have no cut moves", () => {
    const hist = feeHistoryFromOverlay(overlay({ commission: 0 }));
    assert.match(hist.headline, /Unchanged at 0%/);
    assert.equal(hist.lines.length, 0);
    assert.match(hist.emptyLine, /No recorded cut changes/);
  });

  it("explains the cut for someone already staked, and leaves voting off this card", () => {
    const hist = feeHistoryFromOverlay(
      overlay({
        commission: 0,
        stability: {
    ...
    assert.equal(hist.meaning[0].label, "What you keep");
    assert.match(hist.meaning[0].text, /keep all inflation rewards/);
    assert.equal(hist.meaning[1].label, "If it goes up");
    assert.match(hist.meaning[1].text, /Telegram/);
    assert.equal(hist.meaning[2].label, "If it goes down");
    assert.match(hist.meaning[2].text, /already 0%/);
    assert.match(hist.meaning[2].text, /nothing lower/);
    assert.doesNotMatch(JSON.stringify(hist.meaning), /Delinquency|Stability score|voting consistency/);
    assert.equal(hist.votingLines, undefined);
  });

  it("keeps the lower-cut note when there is still room to drop", () => {
    const hist = feeHistoryFromOverlay(overlay({ commission: 8 }));
    assert.match(hist.meaning[0].text, /keep 92%/);
    assert.match(hist.meaning[2].text, /You keep more/);
    assert.doesNotMatch(hist.meaning[2].text, /already 0%/);
  });

  it("compactOverlay keeps fee events for the page", () => {
    const c = compactOverlay({
      vote: "Vote111111111111111111111111111111111AAA",
      commission: 5,
      feeEvents: [{ from: 0, to: 5, captured_at: "2025-01-01T00:00:00.000Z" }],
      stability: { score: 80 }
    });
    assert.equal(c.feeEvents.length, 1);
    assert.equal(c.feeEvents[0].from, 0);
    assert.equal(c.feeEvents[0].to, 5);
  });
});
...
describe("telegram bot URL helper", () => {
  it("defaults to the public stake_health_bot username", () => {
    ...
    assert.equal(TELEGRAM_BOT_URL, "https://telegram.me/stake_health_bot");
    ...
    assert.equal(telegramBotUrl(), "https://telegram.me/stake_health_bot");
    assert.equal(telegramBotUrl(""), "https://telegram.me/stake_health_bot");
    assert.equal(telegramBotUrl("ab"), "https://telegram.me/stake_health_bot");
  });

  it("honors a valid override and strips @", () => {
    assert.equal(telegramBotUsername("@OpenSolanaHubBot"), "OpenSolanaHubBot");
    assert.equal(telegramBotUrl("OpenSolanaHubBot"), "https://telegram.me/OpenSolanaHubBot");
    assert.equal(telegramBotUrl("@OpenSolanaHubBot"), "https://telegram.me/OpenSolanaHubBot");
  });

  it("only accepts https://t.me/<username> or https://telegram.me/<username> bot URLs", () => {
    assert.equal(isTelegramBotUrl("https://t.me/stake_health_bot"), true);
    assert.equal(isTelegramBotUrl("https://telegram.me/stake_health_bot"), true);
    assert.equal(isTelegramBotUrl("https://t.me/OpenSolanaHubBot"), true);
    assert.equal(isTelegramBotUrl("https://telegram.me/OpenSolanaHubBot"), true);
    ...
    assert.equal(isTelegramBotUrl("https://telegram.me/stake_health_bot/extra"), false);
    ...
    assert.equal(isTelegramBotUrl("http://telegram.me/stake_health_bot"), false);
    assert.equal(telegramBotUrl("https://t.me/stake_health_bot"), "https://telegram.me/stake_health_bot");
    assert.equal(
      telegramBotUrl("https://telegram.me/stake_health_bot"),
      "https://telegram.me/stake_health_bot"
    );
  });

  it("builds a /start deep-link from a public key and ignores junk payloads", () => {
    const wallet = "3QPGLackJy5LKctYYoPGmA4P8ncyE197jdxr1zP2ho8K";
    assert.equal(
      telegramStartUrl("https://t.me/stake_health_bot", wallet),
      `https://telegram.me/stake_health_bot?start=${wallet}`
    );
    assert.equal(
      telegramStartUrl(TELEGRAM_BOT_URL, wallet),
      `https://telegram.me/stake_health_bot?start=${wallet}`
    );
    assert.equal(telegramStartUrl(TELEGRAM_BOT_URL, ""), TELEGRAM_BOT_URL);
    assert.equal(telegramStartUrl(TELEGRAM_BOT_URL, "not-a-key"), TELEGRAM_BOT_URL);
    assert.equal(isTelegramStartPayload(wallet), true);
    assert.equal(isTelegramStartPayload("1"), false);
    assert.equal(isTelegramBotUrl(`https://telegram.me/stake_health_bot?start=${wallet}`), false);
  });
...
describe("telegram copy", () => {
  it("status leads with money, includes last-epoch + cumulative, and links mystake", () => {
    ...
    assert.match(text, /Last epochs' rewards:/);
    assert.match(text, /Epoch 841/);
    assert.doesNotMatch(text, /Recent payouts|Consecutive payouts/);
    ...
    assert.match(text, />Fee history</);
    assert.match(text, /#vt-card/);
    ...
    assert.match(text, /Today.s cut: 0%/);
    assert.match(text, /Unchanged at 0%/);
    ...
    const lastIdx = text.indexOf("Last epoch:");
    assert.ok(activeIdx >= 0);
    assert.ok(lastIdx > activeIdx);
    assert.ok(geckoIdx > lastIdx);
    assert.match(text, /Approximate USD from CoinGecko/);
    assert.ok(text.split("\n").length < 26);
  });

  it("Telegram status leads with last epoch, then last epochs' rewards", () => {
    const view = buildHealthView(
      [
        acc({
          validatorName: "AndrewInUA",
    ...
    assert.match(text, /Last epochs' rewards:/);
    assert.match(text, /Epoch 841/);
    assert.doesNotMatch(text, /Consecutive payouts|Since activation|this stake started/);
  });
  ...
  it("Stake story button copy deep-links without a new RPC pull", () => {
    ...
    assert.match(text, />Fee history</);
    assert.doesNotMatch(text, />Full story</);
    assert.match(text, /last epoch, then last epochs' rewards/i);
    assert.ok(text.includes(mystakeUrl(wallet, stake, { story: true }).replace(/&/g, "&amp;")));
    assert.ok(text.includes(mystakeUrl(wallet, stake, { fee: true }).replace(/&/g, "&amp;")));
    assert.match(text, /#full-stake-story/);
    assert.match(text, /Your validator/);
    ...
    assert.match(validatorOnly, />Fee history</);
    assert.match(validatorOnly, /voting and stability/);
    assert.doesNotMatch(validatorOnly, />Stake story</);
    assert.doesNotMatch(validatorOnly, /Today.s cut/);
    assert.equal(validatorOnly.includes(compareUrl(vote)), true);
    assert.doesNotMatch(compareUrl(vote), /what-changed|#vt-card/);
    assert.ok(validatorOnly.includes(mystakeUrl(wallet, null, { fee: true })));
  });

  it("Your validator opens the operator page, not the cut list", () => {
    const wallet = "3QPGLackJy5LKctYYoPGmA4P8ncyE197jdxr1zP2ho8K";
    const vote = "Vote111111111111111111111111111111111AAA";
    const view = buildHealthView(
      [acc()],
    ...
    const text = messages.validatorMessage({
      wallet,
      vote,
      feeHistories: feeHistoriesFromRows(view.rows)
    });
    assert.match(text, /voting and stability/);
    assert.match(text, /Cut history is on Stake health/);
    assert.doesNotMatch(text, /8% → 0%/);
    assert.doesNotMatch(text, /What you keep/);
    assert.doesNotMatch(text, /Today.s cut: 0%/);
    assert.match(text, />Fee history</);
    assert.ok(text.includes(mystakeUrl(wallet, null, { fee: true })));
    assert.equal(text.includes(compareUrl(vote)), true);
    assert.doesNotMatch(compareUrl(vote), /#/);
  });

  it("labels each multi-wallet page link with its public key", () => {
    const first = "HNzoRkVZhiYrZPoV9fUSGBx4wGm1eBm2kHweSUMomYVk";
    const second = "8TEabtrFtRtTtMgaBeUvA74SxjobEaLm5wPpoKQPY1i3";
    const voteA = "Vote111111111111111111111111111111111AAA";
    const voteB = "Vote111111111111111111111111111111111BBB";
    const text = messages.multiWalletPagesMessage(
      [first, second],
      {
        [first]: { vote: voteA },
        [second]: { vote: voteB }
    ...
    assert.match(text, /Operator page for each wallet/);
    assert.match(text, new RegExp(`<code>${first}</code>`));
    assert.match(text, new RegExp(`<code>${second}</code>`));
    assert.match(text, />Fee history</);
    assert.ok(text.includes(mystakeUrl(first, null, { fee: true })));
    assert.ok(text.includes(mystakeUrl(second, null, { fee: true })));
    assert.equal(text.includes(compareUrl(voteA)), true);
    assert.equal(text.includes(compareUrl(voteB)), true);
    assert.doesNotMatch(text, />Stake story</);
    const firstAt = text.indexOf(first);
    const secondAt = text.indexOf(second);
    const voteAAt = text.indexOf(compareUrl(voteA));
    const voteBAt = text.indexOf(compareUrl(voteB));
    assert.ok(firstAt < voteAAt && voteAAt < secondAt && secondAt < voteBAt);
  });
  ...
  it("start copy uses shared How to read this and never asks for a seed", () => {
    ...
    assert.match(start, /Status, Stake story, Your validator, Turn notes on \/ off, Currency, Wallet, Help, Unlink, Stop/);
    assert.match(start, /You control epoch notes with Turn notes on \/ off/);
    assert.match(start, /08:00 UTC/);
    assert.match(start, /Stake story/);
    assert.match(start, />Fee history</);
    assert.match(start, /#vt-card/);
    ...
    assert.match(messages.helpMessage(), /08:00 UTC/);
    assert.match(start, /✅ OK/);
    assert.match(start, /👀 Watch/);
    assert.match(start, /⚠️ Risk/);
    assert.match(messages.helpMessage(), /last epoch, then last epochs' rewards/);
    assert.doesNotMatch(start, /consecutive payouts/i);
    assert.doesNotMatch(messages.helpMessage(), /consecutive payouts/i);
    ...
    assert.match(
      ...
      /This operator's page: voting and stability/
    );
    ...
    assert.match(messages.notifyToggled(true), /raises its cut/);
    assert.match(messages.notifyToggled(true), /08:00 UTC/);
    assert.match(messages.notifyToggled(false), /Epoch notes are off/);
    assert.match(messages.notifyToggled(false), /cut-change notes/);
    assert.match(messages.notifyToggled(false), /Epoch notes are off/);
    assert.match(messages.notifyToggled(false), /\/status still works/);
    assert.equal(messages.notifyToggled(true).includes("—"), false);
    const linkedFromPage = messages.linkedFromPageMessage(
      "3QPGLackJy5LKctYYoPGmA4P8ncyE197jdxr1zP2ho8K",
      "USD",
      true
    );
    assert.match(linkedFromPage, /Linked this wallet from Stake health/);
    assert.match(linkedFromPage, /Epoch notes are on/);
    assert.doesNotMatch(linkedFromPage, /Send \/status/);
    assert.match(messages.walletListMessage([]), /No wallet linked yet/);
    assert.match(
      messages.walletListMessage(["3QPGLackJy5LKctYYoPGmA4P8ncyE197jdxr1zP2ho8K"]),
      /Watching 1 wallet/
    );
    assert.match(messages.helpMessage(), /does not replace/);
    assert.doesNotMatch(
      messages.walletListMessage(["3QPGLackJy5LKctYYoPGmA4P8ncyE197jdxr1zP2ho8K"]),
      /\/unlink|\/stop/
    );
    assert.match(
      messages.confirmDropOneMessage("3QPGLackJy5LKctYYoPGmA4P8ncyE197jdxr1zP2ho8K", 1),
      /Nothing is dropped until you tap Drop it/
    );
    assert.match(
      messages.confirmDropAllMessage(["3QPGLackJy5LKctYYoPGmA4P8ncyE197jdxr1zP2ho8K"]),
      /Nothing is dropped until you tap Drop all/
    );
    assert.match(messages.keptWalletsMessage(), /Nothing was dropped/);
    assert.match(messages.helpMessage(), /asks first/);
    assert.match(messages.helpMessage(), /higher validator cut/);
    assert.match(messages.helpMessage(), /lower cut is on Stake health/);
  });
  ...
  it("Stake health page never pairs CoinGecko with a Signals heading", () => {
    ...
    assert.match(html, /earlier epochs in a row/);
    assert.match(html, /id="verdict-money-story"/);
    assert.match(html, /id="full-stake-story"/);
    assert.match(html, /Last epoch/);
    assert.match(html, /Last epochs' rewards/);
    assert.doesNotMatch(html, />Last payout</);
    ...
    assert.match(js, /setTelegramHandoff/);
    assert.match(js, /Get notes for this wallet/);
    assert.match(js, /telegramStartUrl/);
    ...
    assert.match(js, /Last epochs' rewards/);
    assert.match(js, /not lifetime history/);
    assert.match(js, /We could not follow an earlier payout/);
    ...
    const feeIdx = html.indexOf('id="vt-card"');
    const storyIdx = html.indexOf('id="full-stake-story"');
    const stakesIdx = html.indexOf('id="stakes-card"');
    assert.ok(verdictIdx > 0 && feeIdx > verdictIdx && storyIdx > feeIdx && stakesIdx > storyIdx);
...
describe("telegram reply keyboard and command menu", () => {
  ...
  it("maps slash commands, Menu names, and button labels to the same handlers", () => {
    ...
    assert.deepEqual(bot.parseCommand("/unlink"), { command: "unlink", arg: "" });
    assert.deepEqual(bot.parseCommand("Unlink"), { command: "unlink", arg: "" });
    ...
    assert.deepEqual(
      bot.parseCommand("/start 3QPGLackJy5LKctYYoPGmA4P8ncyE197jdxr1zP2ho8K"),
      { command: "start", arg: "3QPGLackJy5LKctYYoPGmA4P8ncyE197jdxr1zP2ho8K" }
    );
    assert.deepEqual(bot.parseCommand("/start@stake_health_bot 3QPGLackJy5LKctYYoPGmA4P8ncyE197jdxr1zP2ho8K"), {
      command: "start",
      arg: "3QPGLackJy5LKctYYoPGmA4P8ncyE197jdxr1zP2ho8K"
    });
    assert.deepEqual(bot.parseCommand("/start"), { command: "start", arg: "" });
    assert.deepEqual(
      bot.parseCommand("/unlink 3QPGLackJy5LKctYYoPGmA4P8ncyE197jdxr1zP2ho8K"),
      { command: "unlink", arg: "3QPGLackJy5LKctYYoPGmA4P8ncyE197jdxr1zP2ho8K" }
    );
  });
  ...
  it("keeps a persistent newcomer keyboard and BotFather menu list", () => {
    ...
    assert.deepEqual(
      ...
      ["start", "wallet", "status", "currency", "unlink", "stop", "help"]
    );
    const first = "3QPGLackJy5LKctYYoPGmA4P8ncyE197jdxr1zP2ho8K";
    const second = "HNzoRkVZhiYrZPoV9fUSGBx4wGm1eBm2kHweSUMomYVk";
    assert.equal(bot.pickWalletFromText("1 · HNzo…YVk", [first, second]), first);
    assert.equal(bot.pickWalletFromText(second, [first, second]), second);
    assert.equal(bot.isKeepReply("Keep it"), true);
    assert.equal(bot.isDropOneReply("Drop it"), true);
    assert.equal(bot.isDropOneReply("Drop all"), false);
    assert.equal(bot.isDropAllReply("Drop all"), true);
    assert.equal(bot.unlinkTarget(`unlink:${first}`), first);
    assert.equal(JSON.stringify(kb).includes("—"), false);
    assert.equal(bot.messageReplyMarkup({ notifyEpoch: true }, {}), undefined);
    assert.equal(
      bot.messageReplyMarkup({ notifyEpoch: true }, { showKeyboard: true }).keyboard[0][0].text,
      "Status"
    );
  });
});
...
describe("telegram epoch notify flag", () => {
  ...
  it("epoch cron skips notify-off chats and still sends to notify-on", () => {
    const subs = [
      ...
      { chatId: "4", wallet: "3QPGLackJy5LKctYYoPGmA4P8ncyE197jdxr1zP2ho8K" },
      {
        chatId: "5",
        wallet: null,
        wallets: ["3QPGLackJy5LKctYYoPGmA4P8ncyE197jdxr1zP2ho8K"],
        notifyEpoch: true
      }
    ];
    ...
    assert.deepEqual(
      digest.commissionWatchTargets(subs).map(s => s.chatId),
      ["1", "4", "5"]
    );
  });

  it("pings only when a known validator cut goes up", () => {
    const vote = "3QPGLackJy5LKctYYoPGmA4P8ncyE197jdxr1zP2ho8K";
    const first = digest.commissionRaises({}, [{ vote, name: "AndrewInUA", commission: 0 }]);
    assert.deepEqual(first.raises, []);
    assert.equal(first.lastCommissionByVote[vote], 0);
    const raise = digest.commissionRaises(first.lastCommissionByVote, [
      { vote, name: "AndrewInUA", commission: 8 }
    ]);
    assert.equal(raise.raises.length, 1);
    assert.equal(raise.raises[0].from, 0);
    assert.equal(raise.raises[0].commission, 8);
    const drop = digest.commissionRaises(raise.lastCommissionByVote, [
      { vote, name: "AndrewInUA", commission: 5 }
    ]);
    assert.deepEqual(drop.raises, []);
    assert.equal(drop.lastCommissionByVote[vote], 5);
    const text = messages.commissionRaiseMessage(raise.raises);
    assert.match(text, /AndrewInUA/);
  ...
  it("adds a second wallet instead of replacing the first", () => {
    const first = "3QPGLackJy5LKctYYoPGmA4P8ncyE197jdxr1zP2ho8K";
    const second = "HNzoRkVZhiYrZPoV9fUSGBx4wGm1eBm2kHweSUMomYVk";
    const sub = store.emptySub("9");
    assert.deepEqual(store.linkedWallets({ wallet: first }), [first]);
    const added = store.addWallet(sub, first);
    assert.equal(added.ok, true);
    const again = store.addWallet(sub, second);
    assert.equal(again.ok, true);
    assert.deepEqual(store.linkedWallets(sub), [first, second]);
    assert.equal(sub.wallet, first);
    const dup = store.addWallet(sub, first);
    assert.equal(dup.reason, "duplicate");
    store.removeWallet(sub, first);
    assert.deepEqual(store.linkedWallets(sub), [second]);
    assert.equal(sub.wallet, second);
  });

  it("forgets a raise baseline when that validator is no longer on the chat", () => {
    const first = "3QPGLackJy5LKctYYoPGmA4P8ncyE197jdxr1zP2ho8K";
    const second = "HNzoRkVZhiYrZPoV9fUSGBx4wGm1eBm2kHweSUMomYVk";
    const voteA = "Vote111111111111111111111111111111111AAA";
    const voteB = "Vote111111111111111111111111111111111BBB";
    const sub = store.emptySub("11");
    store.addWallet(sub, first);
    store.addWallet(sub, second);
    sub.lastByWallet = {
      [first]: { vote: voteA, votes: [voteA] },
      [second]: { vote: voteB, votes: [voteB] }
    };
    sub.lastCommissionByVote = { [voteA]: 0, [voteB]: 7 };
    store.removeWallet(sub, first);
    assert.equal(sub.lastCommissionByVote[voteA], undefined);
    assert.equal(sub.lastCommissionByVote[voteB], 7);
    store.removeWallet(sub, second);
    assert.deepEqual(sub.lastCommissionByVote, {});
  });
});
...
describe("telegram-info public endpoint", () => {
  ...
  it("returns a telegram.me URL from TELEGRAM_BOT_USERNAME and never a token", async () => {
    ...
    assert.equal(json.url, "https://telegram.me/OpenSolanaHubBot");
  ...
  it("defaults to stake_health_bot when username is unset", async () => {
    ...
    assert.equal(json.url, "https://telegram.me/stake_health_bot");
...
describe("mystake.html static contract", () => {
  ...
  it("hardcodes the Telegram bot and newcomer OK copy", () => {
    assert.match(html, /id="nav-telegram"[^>]*href="https:\/\/telegram\.me\/stake_health_bot"/);
    assert.match(html, /id="telegram-open"[^>]*href="https:\/\/telegram\.me\/stake_health_bot"/);
    ...
    assert.match(html, /Keep watch in Telegram/);
    assert.match(html, /up to 5 wallets/);
    assert.match(html, /08:00 UTC/);
    assert.match(html, /raises its cut|cut went up/);
    assert.match(html, /id="vt-card"/);
    assert.match(html, /id="vt-headline"/);
    assert.match(html, /id="vt-history"/);
    assert.match(html, />Fee history</);
    assert.match(html, /fee-history-pattern/);
    assert.match(html, /share of inflation rewards/);
    assert.match(html, /id="vt-open"[^>]*target="_blank"/);
    assert.match(html, /id="vt-open"[^>]*rel="noopener noreferrer"/);
    assert.match(html, /Open Validator Transparency/);
    assert.match(html, /Example:/);
    assert.match(html, /Your validator raised the cut: 0% → 8%/);
    assert.doesNotMatch(html, /AndrewInUA raised/);
    assert.doesNotMatch(TELEGRAM_CTA.raiseExample, /AndrewInUA/);
    assert.match(html, /lower cut stays here/);
  ...
  it("page JS only applies a Telegram bot URL and never a hash href", () => {
    assert.match(pageJs, /safeTelegramBotUrl/);
    assert.match(pageJs, /isTelegramBotUrl\(candidate\)/);
    assert.match(pageJs, /feeHistoryFromOverlay/);
    assert.match(pageJs, /function openAway/);
    assert.match(pageJs, /Detailed history/);
    assert.match(pageJs, /profileHref\(vote, "what-changed-card"\)/);
    assert.doesNotMatch(pageJs, /profileHref\(v\.vote,\s*["']what-changed-card["']\)/);
    assert.doesNotMatch(pageJs, /profileHref\(acc\.vote,\s*["']what-changed-card["']\)/);
    assert.doesNotMatch(coreJs, /what-changed-card/);
    assert.doesNotMatch(pageJs, /href\s*=\s*["']#["']/);
    assert.ok(coreJs.includes("telegram.me"));
    assert.ok(coreJs.includes("/^https:\\/\\/(?:t\\.me|telegram\\.me)\\/[A-Za-z0-9_]{5,32}$/"));
  });
