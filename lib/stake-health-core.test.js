...
describe("telegram copy", () => {
  ...
  it("start copy uses shared How to read this and never asks for a seed", () => {
    ...
    assert.match(start, /Status, Currency, Wallet, Help, Stop/);
  ...
  it("Stake health page keeps FX copy with Approx., not the Signals kicker", () => {
    ...
    const controlBlock = html.slice(html.indexOf('id="fiat-control"'), html.indexOf('id="stakes-list"'));
    assert.match(controlBlock, /id="fiat-hint"/);
    assert.match(controlBlock, /id="fiat-select"/);
    const kickerBlock = html.slice(html.indexOf('id="stakes-kicker"'), html.indexOf('id="fiat-control"'));
    assert.doesNotMatch(kickerBlock, /fiat-hint/);
    assert.match(html, /id="verdict-fiat"/);
    assert.match(js, /placeFiatControl/);
...
describe("telegram reply keyboard and command menu", () => {
  ...
  it("keeps a persistent newcomer keyboard and BotFather menu list", () => {
    ...
    assert.deepEqual(labels, ["Status", "Currency", "Wallet", "Help", "Stop"]);
    assert.deepEqual(
