...
describe("telegram copy", () => {
  ...
  it("Your validator lists recorded cut moves, newest first", () => {
    ...
    assert.equal(text.includes(compareUrl(vote)), true);
    assert.match(text, /lower cut stays here/);
  });
  ...
  it("start copy uses shared How to read this and never asks for a seed", () => {
    ...
    assert.match(
      ...
      /today’s cut and recorded moves/
    );
    ...
    assert.match(messages.helpMessage(), /lower cut is on Your validator/);
  });
...
describe("mystake.html static contract", () => {
  ...
  it("hardcodes the Telegram bot and newcomer OK copy", () => {
    ...
    assert.match(html, /own Telegram message/);
    assert.match(html, /raised the validator.s cut: 0% → 8%/);
