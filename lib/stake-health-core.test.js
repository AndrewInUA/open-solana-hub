...
describe("verdict semantics (shared with mystake.js)", () => {
  ...
  it("How to read this matches the shared tone bodies", () => {
    ...
    const blob = [
      ...
      formatRewardEpochLine({ epoch: 1029, recorded: false, amountSol: null })
    ].join("\n");
...
describe("stake money picture (shared with mystake.js)", () => {
  ...
  it("does not treat a missing last epoch as 0 SOL earned", () => {
    ...
    assert.ok(summary.rewardRows.every(r => r.recorded === false && r.amountSol === null));
  });
  ...
  it("labels a full window since activation and a partial window honestly", () => {
    ...
    assert.equal(
      since.windowLabel,
      "Recent rewards – last 2 finished epochs (we look back up to 16)"
    );
    ...
    assert.equal(windowed.windowSize, 16);
    assert.equal(windowed.recordedCount, 2);
    assert.equal(
      windowed.windowLabel,
      "Recent rewards – last 16 finished epochs (we look back up to 16)"
    );
    assert.match(moneyStory(windowed, { perSol: { USD: 150 } }, "USD"), /Not the full time since you delegated/);
    assert.match(moneyStory(windowed, { perSol: { USD: 150 } }, "USD"), /recorded rewards/);
    assert.doesNotMatch(moneyStory(windowed, { perSol: { USD: 150 } }, "USD"), /last 2 finished epochs/);
  });

  it("keeps a consecutive epoch list and does not drop gaps", () => {
    const summary = summarizeAccountRewards(
      acc({
        activationEpoch: 100,
        rewards: [
    ...
    assert.equal(summary.windowSize, 16);
    assert.equal(summary.recordedCount, 4);
    assert.equal(summary.rewardRows.length, 16);
    assert.deepEqual(
      summary.rewardRows.map(r => r.epoch),
      [1031, 1030, 1029, 1028, 1027, 1026, 1025, 1024, 1023, 1022, 1021, 1020, 1019, 1018, 1017, 1016]
    );
    const byEpoch = Object.fromEntries(summary.rewardRows.map(r => [r.epoch, r]));
    assert.equal(byEpoch[1031].recorded, true);
    assert.equal(byEpoch[1029].recorded, false);
    assert.equal(byEpoch[1029].amountSol, null);
    assert.equal(formatRewardEpochLine(byEpoch[1031]), "Epoch 1031  +0.012 SOL");
    assert.equal(formatRewardEpochLine(byEpoch[1029]), "Epoch 1029  No reward recorded");
    assert.equal(
    ...
    assert.ok(Math.abs(summary.cumulativeSol - 0.04) < 1e-9);
    const window = consecutiveRewardWindow(
      acc({
        activationEpoch: 100,
        rewards: summary.rewardRows.filter(r => r.recorded)
      }),
      1032
    );
  ...
  it("sums getInflationReward rows and never fills a failed epoch with 0", async () => {
    ...
    assert.equal(summary.fromActivation, true);
    assert.equal(summary.rewardRows.find(r => r.epoch === 840)?.recorded, false);
    assert.equal(summary.rewardRows.find(r => r.epoch === 840)?.amountSol, null);
    assert.match(formatRewardEpochLine(summary.rewardRows.find(r => r.epoch === 840)), /No reward recorded/);
  });
  ...
  it("money story and telegram lines stay on Approx. copy, with en dashes", () => {
    ...
    assert.match(lines[2], /^Recent rewards – last 2 finished epochs \(we look back up to 16\):/);
    assert.doesNotMatch(lines.join("\n"), /Since activation/);
    assert.equal(MAX_REWARD_HISTORY_EPOCHS, 16);
    assert.equal(core.REWARD_RPC_CONCURRENCY, 16);
    assert.equal(NO_REWARD_RECORDED, "No reward recorded");
  });
});
...
describe("telegram copy", () => {
  it("status leads with money, includes last-epoch + cumulative, and links mystake", () => {
    ...
    assert.match(text, /Recent rewards – last 2 finished epochs \(we look back up to 16\)/);
    assert.doesNotMatch(text, /Since activation/);
    ...
    const recentIdx = text.indexOf("Recent rewards – last 2 finished epochs");
  ...
  it("start copy uses shared How to read this and never asks for a seed", () => {
    ...
    assert.match(start, /Status, Stake story, Your validator, Notify, Currency, Wallet, Help, Stop/);
    ...
    assert.match(messages.helpMessage(), /Stake story/);
  ...
  it("Stake health page never pairs CoinGecko with a Signals heading", () => {
    ...
    assert.match(js, /epoch-reward-empty/);
    assert.match(html, /No reward recorded/);
    assert.match(html, /we look back up to 16/);
