...
async function loadStakeHealthMany(wallets) {
  ...
  for (const wallet of keys) {
    try {
      packs.push({ wallet, pack: await resolvePositions({ wallet, stake: "" }) });
    } catch (err) {
      packs.push({ wallet, error: err });
    }
  }
  ...
  if (uniqueAccounts.length && Number.isFinite(currentEpoch)) {
    try {
  ...
  if (votes.length) {
    try {
      overlays = await loadOverlays(votes);
    } catch {
      overlays = null;
    }
  }
  ...
  for (const wallet of keys) {
    try {
      ...
      for (const acc of pack.accounts || []) {
        ...
        seen.set(vote, wallet);
      }
    } catch {
      /* skip a wallet that failed to resolve */
    }
  }
