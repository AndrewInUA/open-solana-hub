 ...
 *      finished epochs may be fetched; display is a consecutive run from
 *      last epoch – never a gappy list, never filled with 0.
...
function rpcUrls() {
  ...
  if (solanaRpc) urls.push(solanaRpc);
...
function orderedRpcUrls() {
  const urls = rpcUrls();
  if (preferredRpc && urls.includes(preferredRpc)) {
    return [preferredRpc, ...urls.filter(url => url !== preferredRpc)];
  }
  return urls;
}
...
/**
 * Full Stake health checkup for one or more wallets (same join as mystake.js).
 * Extra epoch payouts are fetched once for every stake account together, so a
 * second wallet does not lose last-epochs' rewards when RPC is tired.
 * Overlays fail soft: we still return stake totals and a weaker verdict.
 */
function readHealthCache(wallet) {
  const hit = healthCache.get(wallet);
  if (!hit || Date.now() - hit.at > HEALTH_TTL_MS) return null;
  return hit.entry;
}

function writeHealthCache(entry) {
  if (!entry?.wallet || !entry.view) return;
  healthCache.set(entry.wallet, { at: Date.now(), entry });
}

async function loadStakeHealthMany(wallets) {
  const keys = [];
  for (const item of wallets || []) {
    const key = String(item || "").trim();
    if (!isPubkey(key) || keys.includes(key)) continue;
    keys.push(key);
  }
  const cached = new Map();
  const missing = [];
  for (const key of keys) {
    const hit = readHealthCache(key);
    if (hit) cached.set(key, hit);
    else missing.push(key);
  }
  const fresh = missing.length ? await loadStakeHealthUncached(missing) : [];
  for (const entry of fresh) writeHealthCache(entry);
  const byWallet = new Map(cached);
  for (const entry of fresh) byWallet.set(entry.wallet, entry);
  return keys.map(key => byWallet.get(key) || { wallet: key, error: new Error("Lookup failed") });
}

async function loadStakeHealthUncached(keys) {
  const fiatP = fetchSolFiatRates().catch(() => null);
  const packs = await Promise.all(
    keys.map(async wallet => {
      try {
        return { wallet, pack: await resolvePositions({ wallet, stake: "" }) };
      } catch (error) {
        return { wallet, error };
      }
    })
  ...
  for (const row of packs) {
    if (row.pack?.currentEpoch != null && currentEpoch == null) {
      currentEpoch = Number(row.pack.currentEpoch);
    }
    for (const acc of row.pack?.accounts || []) {
      if (acc?.pubkey && !byPubkey.has(acc.pubkey)) byPubkey.set(acc.pubkey, acc);
    }
  }
  const uniqueAccounts = [...byPubkey.values()];
  const votes = uniqueAccounts.map(acc => acc.vote).filter(Boolean);
  const rewardP = (async () => {
    if (!uniqueAccounts.length || !Number.isFinite(currentEpoch)) return;
    try {
      const enriched = await fetchRewardHistory(rpcCall, uniqueAccounts, currentEpoch, {
        concurrency: 8,
        retries: 1
      });
      const mapped = new Map(enriched.map(acc => [acc.pubkey, acc]));
      for (const row of packs) {
        if (!row.pack) continue;
        row.pack.accounts = (row.pack.accounts || []).map(acc => mapped.get(acc.pubkey) || acc);
      }
    } catch {
      /* keep last-epoch rewards from /api/my-stake */
    }
  })();
  const overlayP = votes.length ? loadOverlays(votes).catch(() => null) : Promise.resolve(null);
  const [, overlays, rates] = await Promise.all([rewardP, overlayP, fiatP]);
  return packs.map(row => {
    if (row.error) return { wallet: row.wallet, error: row.error };
    const accounts = row.pack?.accounts || [];
    const view = buildHealthView(accounts, overlays, row.pack);
    return { wallet: row.wallet, view: { pack: row.pack, rates, ...view } };
  });
}

async function loadCommissionReadings(wallets) {
  const keys = [];
  for (const item of wallets || []) {
    const key = String(item || "").trim();
    if (!isPubkey(key) || keys.includes(key)) continue;
  ...
  for (const row of packs) {
    ...
    for (const acc of row.pack.accounts || []) {
      ...
      seen.set(vote, row.wallet);
    }
  }
  const overlays = await loadOverlays([...seen.keys()]);
  const out = [];
  for (const [vote, wallet] of seen) {
    const overlay = overlays.get(vote);
...
async function loadStakeHealth(wallet) {
  ...
  const [entry] = await loadStakeHealthMany([key]);
  if (entry?.error) throw entry.error;
  if (!entry?.view) throw new Error("Lookup failed");
  return entry.view;
}
