...
function hasValidatorContext(wallets, contexts) {
  return (wallets || []).every(wallet => {
    const ctx = contexts?.[wallet];
    if (!ctx) return false;
    if (ctx.vote) return true;
    return Array.isArray(ctx.votes) && ctx.votes.length > 0;
  });
}
...
async function sendValidator(chatId, sub) {
  ...
  if (wallets.length > 1) {
    if (!hasValidatorContext(wallets, sub.lastByWallet)) {
      const entries = await loadWalletViews(wallets);
      rememberStories(sub, entries);
      sub.updatedAt = new Date().toISOString();
      await store.saveSub(sub).catch(() => null);
