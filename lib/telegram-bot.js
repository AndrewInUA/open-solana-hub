...
async function handleText(chat, from, text) {
  ...
  if (command === "stop" || (command === "unlink" && !arg)) {
  ...
  if (command === "unlink") {
    if (!isPubkey(arg)) {
      await reply("Paste the public key to drop, or send /stop to drop all.");
      return;
    }
    const before = store.linkedWallets(sub);
    if (!before.includes(arg)) {
      await reply("That wallet is not on this chat.");
      return;
    }
    store.removeWallet(sub, arg);
    sub.awaiting = null;
    sub.updatedAt = new Date().toISOString();
    await store.saveSub(sub);
    await send(chatId, msg.unlinkedOneMessage(arg, store.linkedWallets(sub)), { sub });
    return;
  }
