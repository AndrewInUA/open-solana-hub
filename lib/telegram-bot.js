...
async function handleText(chat, from, text) {
  ...
  if (command === "start") {
    ...
    if (fromPage) {
      sub = await saveWallet(chatId, from, String(arg).trim(), { notesOn: true });
      await send(chatId, msg.linkedFromPageMessage(sub.wallet, sub.fiat, store.isNotifyEnabled(sub)), {
        sub
      });
