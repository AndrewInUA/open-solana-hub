...
/**
 * Reply keyboards stay on screen until replaced. Re-sending the main keyboard
 * with a long note makes Telegram draw that note under the panel. Attach it
 * only when the buttons must appear or change (`showKeyboard` / `reply_markup`).
 */
function messageReplyMarkup(sub, { reply_markup, showKeyboard } = {}) {
  if (reply_markup) return reply_markup;
  if (showKeyboard) return mainKeyboard(sub);
  return undefined;
}
...
async function send(chatId, text, extra) {
  const rest = { ...(extra || {}) };
  const sub = rest.sub;
  const showKeyboard = rest.showKeyboard;
  delete rest.sub;
  delete rest.showKeyboard;
  const reply_markup = messageReplyMarkup(sub, {
...
function rememberStories(sub, entries) {
  ...
  for (const entry of ok) {
    const ctx = storyContextFromViewSafe(entry.view, entry.wallet);
    sub.lastByWallet[entry.wallet] = {
      stake: ctx.stake || null,
      vote: ctx.vote || null,
      votes: (ctx.votes || []).map(item => item.vote).filter(Boolean)
    };
  }
...
async function loadWalletViews(wallets) {
  return lookup.loadStakeHealthMany(wallets);
}
...
async function sendStatus(chatId, sub) {
  await telegram("sendChatAction", { chat_id: chatId, action: "typing" }).catch(() => null);
  const wallets = store.linkedWallets(sub);
  if (!wallets.length) {
    await send(chatId, msg.noWalletMessage(), { sub, showKeyboard: true });
    return;
  }
...
async function sendStory(chatId, sub) {
  ...
  await send(
    chatId,
    msg.multiWalletPagesMessage(wallets, sub.lastByWallet, { kind: "story" }),
    { sub }
  );
}
...
async function sendValidator(chatId, sub) {
  ...
  await send(
    chatId,
    msg.multiWalletPagesMessage(wallets, sub.lastByWallet, { kind: "validator" }),
    { sub }
  );
}
...
async function toggleNotify(chatId, sub, arg) {
  ...
  if (!sub.createdAt) sub.createdAt = sub.updatedAt;
  await store.saveSub(sub);
  await send(chatId, msg.notifyToggled(store.isNotifyEnabled(sub)), { sub, showKeyboard: true });
}
...
async function handleText(chat, from, text) {
  ...
  if (command === "start") {
    ...
    if (fromPage) {
      ...
      if (result.reason === "full") {
        await send(chatId, msg.walletFullMessage(result.wallets), { sub, showKeyboard: true });
        return;
      }
    ...
    await store.saveSub(sub);
    await send(chatId, msg.startMessage(), { sub, showKeyboard: true });
  ...
  async function dropOne(wallet) {
    ...
    sub.updatedAt = new Date().toISOString();
    await store.saveSub(sub);
    await send(chatId, msg.unlinkedOneMessage(wallet, store.linkedWallets(sub)), {
      sub,
      showKeyboard: true
    });
  }
  ...
  async function keepWallets() {
    ...
    sub.updatedAt = new Date().toISOString();
    await store.saveSub(sub);
    await send(chatId, msg.keptWalletsMessage(), { sub, showKeyboard: true });
  }
  ...
  if (command === "wallet") {
    ...
    if (result.reason === "duplicate") {
      await send(chatId, msg.alreadyWatchingMessage(arg, result.wallets), { sub, showKeyboard: true });
      return;
    }
    await send(
      chatId,
  ...
  if (isPubkey(text)) {
    ...
    if (result.reason === "duplicate") {
      await send(chatId, msg.alreadyWatchingMessage(text, result.wallets), { sub, showKeyboard: true });
      return;
    }
    await send(
      chatId,
