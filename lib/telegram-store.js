...
function removeWallet(sub, wallet) {
  ...
  applyWallets(sub, list);
  pruneCommissionWatch(sub);
  return { ok: true, sub, wallets: list };
}

function watchedVotes(sub) {
  const votes = new Set();
  const byWallet =
    sub?.lastByWallet && typeof sub.lastByWallet === "object" ? sub.lastByWallet : {};
  for (const wallet of linkedWallets(sub)) {
    const ctx = byWallet[wallet] || {};
    if (ctx.vote) votes.add(String(ctx.vote));
    for (const vote of ctx.votes || []) {
      if (vote) votes.add(String(vote));
    }
  }
  return votes;
}

/** Drop raise-watch baselines for validators no longer on this chat. Relink starts over. */
function pruneCommissionWatch(sub) {
  if (!sub || typeof sub !== "object") return sub;
  if (!linkedWallets(sub).length) {
  ...
  const keep = watchedVotes(sub);
  if (!keep.size) return sub;
  const prev =
    sub.lastCommissionByVote &&
    typeof sub.lastCommissionByVote === "object" &&
    !Array.isArray(sub.lastCommissionByVote)
      ? sub.lastCommissionByVote
      : {};
  const next = {};
  for (const [vote, value] of Object.entries(prev)) {
    if (keep.has(vote)) next[vote] = value;
  }
  sub.lastCommissionByVote = next;
  if (sub.lastByWallet && typeof sub.lastByWallet === "object") {
    const kept = {};
    for (const wallet of linkedWallets(sub)) {
      if (sub.lastByWallet[wallet]) kept[wallet] = sub.lastByWallet[wallet];
    }
    sub.lastByWallet = kept;
  }
  return sub;
}
